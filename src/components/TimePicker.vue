<template>
  <div class="time-picker">
    ...
    <div v-show="visible">
      ...
      <ul class="flex-container" v-if="values.hh">
  ...
  export default {
    ...
    created () {
      if (this.value && this.value.match(/^[0-9]{1,2}:[0-9]{1,2}$/)) {
        ...
        this.values.mm = this.round(this.value.split(':')[1]).toString().padStart(2, '0')
      }
    },
    ...
    watch: {
      value (val) {
        if (val && val.match(/^[0-9]{1,2}:[0-9]{1,2}$/)) {
          ...
          this.values.mm = this.round(val.split(':')[1]).toString().padStart(2, '0')
        }
      }
    },
