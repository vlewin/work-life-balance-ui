<template>
  <div class="input-time">
    <div class="input-time-field hours" v-for="(hour, index) in hours" v-bind:class="{ selected: selected == index, set: isDirty(hour) }">
    ...
    <div class="input-time-field minutes" v-for="(minute, index) in minutes"  v-bind:class="{ selected: selected == index+2, set: isDirty(minute) }">
      <div class="input-time-field-item">
        {{ minute.v }}
      </div>
      <div class="input-time-field-item">
    ...
    watch: {
      initValue(val) {
        if(val) {
          ...
          val.replace(':', '').split('').forEach((v, index) => {
      ...
      value (val) {
        if (val) {
          console.log('InputTime - value change', val)

          const value =  val.replace(':', '')
          const index = value.length - 1

          this.fields[index].n = value[index]
          this.selected += 1
...
<style scoped>
  ...
  .input-time-field {
    ...
    /*transition: flex 0.2s, opacity 0.2;*/
    width: 2rem;
    height: 3rem;
    text-align: center;
  }
  ...
  .input-time-field.set .input-time-field-item {
